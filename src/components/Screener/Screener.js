...
const Screener = () => {
  ...
  const shouldShowRow = (symbolDataItem) => {
    if (tradeStatus === null) {
      return true;
    } else if (tradeStatus === "buy") {
      return (
        Math.abs(
          symbolDataItem.regularMarketOpen - symbolDataItem.regularMarketDayLow
        ) < 0.0001
      );
    } else if (tradeStatus === "sell") {
      return (
        Math.abs(
          symbolDataItem.regularMarketOpen - symbolDataItem.regularMarketDayHigh
        ) < 0.0001
      );
    } else if (tradeStatus === "wait for trade") {
      // Show rows where neither "buy" nor "sell" conditions are met
      const buyCondition =
        Math.abs(
          symbolDataItem.regularMarketOpen - symbolDataItem.regularMarketDayLow
        ) < 0.0001;
    ...
    // Add other conditions for additional trade statuses, if needed
